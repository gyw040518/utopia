...
if django.get_version() != '1.7.1':
    setup = django.setup()

from upuser.user_api import db_add_user
from upuser.models import User
from utopia.api import get_object
from install import color_print
from utopia.api import get_mac_address
...
#        os.chmod('keys', 0777)
    ...
    def _create_admin(self):
        ...
        db_add_user(username=self.admin_user, password=self.admin_pass, role='SU', name='admin', groups='',
                    admin_groups='', email='admin@utopia.org', uuid='MayBeYouAreTheFirstUser', is_active=True)
        os.system('id %s &> /dev/null || useradd %s' % (self.admin_user, self.admin_user))

    @staticmethod
    def _cp_zzsh():
        os.chdir(os.path.join(jms_dir, 'install'))
        shutil.copy('zzutopia.sh', '/etc/profile.d/')
    ...
    def start(self):
        print "开始安装Utopia, 要求环境为 CentOS 6.5 x86_64"
        self._pull()
        self._sync_db()
        self._input_admin()
        self._create_admin()
