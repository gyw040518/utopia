...
from upuser.models import AdminGroup
from utopia.api import *
from utopia.settings import BASE_DIR, EMAIL_HOST_USER as MAIL_FROM
...
def user_add_mail(user, kwargs):
    ...
    mail_title = u'恭喜你的统一管理平台用户 %s 添加成功 统一管理平台' % user.name
    mail_msg = u"""
    Hi, %s
        您的用户名： %s
        您的权限： %s
        您的web登录密码： %s
        您的ssh密钥文件密码： %s
        密钥下载地址： %s/upuser/key/down/?uuid=%s
        说明： 请登陆后再下载密钥！
...
def server_del_user(username):
    """
    delete a user from utopia linux system
    删除系统上的某用户
    """
...
def get_display_msg(user, password, ssh_key_pwd, ssh_key_login_need, send_mail_need):
    ...
    if ssh_key_login_need:
        ...
        密码：%s
        密钥密码：%s
        密钥下载url: %s/upuser/key/down/?uuid=%s
        该账号密码可以登陆web和统一管理平台。
