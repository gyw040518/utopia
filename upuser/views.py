...
from upperm.perm_api import get_group_user_perm
...
def group_add(request):
    ...
    return my_render('upuser/group_add.html', locals(), request)
...
def group_list(request):
    ...
    return my_render('upuser/group_list.html', locals(), request)
...
def group_edit(request):
    ...
    return my_render('upuser/group_edit.html', locals(), request)
...
def user_add(request):
    ...
    return my_render('upuser/user_add.html', locals(), request)
...
def forget_password(request):
    if request.method == 'POST':
        ...
        if user:
            ...
            send_mail('忘记统一管理平台密码', msg, MAIL_FROM, [email], fail_silently=False)
            msg = u'请登陆邮箱，点击邮件重设密码'
            return http_success(request, msg)
        else:
            error = u'用户不存在或邮件地址错误'

    return render_to_response('upuser/forget_password.html', locals())
...
def reset_password(request):
    ...
    action = '/upuser/password/reset/?uuid=%s&timestamp=%s&hash=%s' % (uuid_r, timestamp, hash_encode)
    ...
    if hash_encode == PyCrypt.md5_crypt(uuid_r + timestamp + KEY):
        if int(time.time()) - int(timestamp) > 600:
            ...
            return render_to_response('upuser/reset_password.html', locals())
...
def user_edit(request):
    ...
    if request.method == 'GET':
        ...
        return HttpResponseRedirect(reverse('user_list'))
    return my_render('upuser/user_edit.html', locals(), request)
...
def profile(request):
    ...
    return my_render('upuser/profile.html', locals(), request)
...
def change_info(request):
    ...
    return my_render('upuser/change_info.html', locals(), request)
