...
from upperm.perm_api import user_have_perm
from django.http import HttpResponseNotFound
from uplog.log_api import renderTemplate

from uplog.models import Log, ExecLog, FileLog
from utopia.settings import WEB_SOCKET_HOST
...
def log_list(request, offset):
    ...
    return render_to_response('uplog/log_%s.html' % offset, locals(), context_instance=RequestContext(request))


@require_role('admin')
def log_detail(request):
    return my_render('uplog/exec_detail.html', locals(), request)
...
def log_kill(request):
    ...
    if log:
        ...
        return render_to_response('uplog/log_offline.html', locals(), context_instance=RequestContext(request))
    else:
...
def log_detail(request, offset):
    ...
    if offset == 'exec':
        ...
        try:
            ...
            result = {}
        return my_render('uplog/file_detail.html', locals(), request)
